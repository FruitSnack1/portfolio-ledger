...
import type { AssetAddLogMonthOption } from './AssetAddLogModal'

type AssetEditLogModalProps = {
  open: boolean
  onClose: () => void
  year: number
  month: number
...
export function AssetEditLogModal({
  open,
  onClose,
  year,
  month,
  ...
  return (
    <dialog ref={dialogRef} className="modal-dialog asset-add-log-modal" aria-labelledby="asset-edit-log-title">
      <div className="modal-wrap" onClick={backdropClick}>
        <div className="modal-panel asset-add-log-modal-panel" role="document">
          <h2 id="asset-edit-log-title" className="modal-title">
            Edit log
          </h2>
          ...
          <form className="form" onSubmit={(ev) => void onSubmit(ev)}>
            ...
            {fieldsDisabled && (
              <p className="hint">Finish or cancel the asset edit or add-log flow before editing this log.</p>
            )}
